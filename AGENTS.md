...
src/models/*.rs       # your Sea-ORM model logic (users.rs, vault.rs, vault_sub_item.rs, etc.)
src/streaming/        # MediaProcessor & VideoProcessor (FFmpeg transcoding, track/sub/font extraction, monitoring)
...
src/dtos/             # Domain DTOs & ts-rs types exported to frontend/src/bindings/
...
migration/            # Sea-ORM migrations & schema.sql
...
- Primary keys on `users`, `vault`, `vault_sub_item`, etc. are **UUID v7** (`uuid::Uuid`), not sequential integers.
...
- **Shared store state**: `ctx.shared_store` holds:
  ...
  - `Arc<MediaProcessor>`: Media post-processor and transcoding manager. Retrieve with `ctx.shared_store.get::<Arc<MediaProcessor>>().unwrap()`.
  - `Arc<DownloadManager>`: Central download orchestrator. Retrieve with `ctx.shared_store.get::<Arc<DownloadManager>>().unwrap()`.
  - `Sender<AppEvent>`: Tokio broadcast channel for real-time Server-Sent Events (SSE) and media streaming progress. Retrieve with `ctx.shared_store.get::<broadcast::Sender<AppEvent>>().unwrap()`.
